...
import Link from "next/link";
import toast from "react-hot-toast";
import { FaArrowLeft, FaSearch, FaPrint } from "react-icons/fa";
import { FiChevronsLeft, FiChevronsRight } from "react-icons/fi";
import { ChevronLeft, ChevronRight } from "lucide-react";
import DatePicker from "react-datepicker";
import "react-datepicker/dist/react-datepicker.css";
...
const BrokerReportTable: React.FC<TransactionTableProps> = () => {
  ...
  useEffect(() => {
    if (startDate && endDate) {
      const filtered = transactionRecord.filter((record: any) => {
        const recordDate = new Date(record.createdAt); // Adjust if your date is in different format
        return recordDate >= startDate && recordDate <= endDate;
      });
      setFilteredData(filtered);
  ...
  return (
    <div className="transcations-record-page-wrapper px-10 flex flex-col justify-center pt-[45px]">
      ...
      <div className="pagination-wrapper flex justify-end gap-3 mt-3">
        <button
          onClick={goToFirstPage}
