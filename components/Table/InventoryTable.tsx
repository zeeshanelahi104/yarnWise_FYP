...
import { FaPen, FaPrint,FaSearch,FaArrowLeft,FaChevronLeft,FaChevronRight } from "react-icons/fa";
import { MdDelete } from "react-icons/md";
import Image from "next/image";
import { useDeleteInventoryMutation, useGetInventoriesQuery } from "@/features/inventorySlice";
...
const InventoryTable: React.FC<InventoryTableProps> = () => {
    ...
    const handleDeleteInventory = (id:string) => {
      deleteInventory(id).unwrap().then(()=> { toast.success("Inventory Deleted")}
      ).catch(()=> { toast.error("Error, Deleting Inventory")})
    }
            ...
            <TableBody>
              {currentItems.map((inventory:Inventory, index:number) => (
                <TableRow key={index}>
                  ...
                  {inventory?._id ? inventory._id.substring(inventory._id.length - 4).toUpperCase() : ''}
                  </TableCell>
                  ...
                  <TableCell className="border-2 border-black">
                    ...
                    <Link href={`/inventory/editproduct/${inventory?._id}`}><FaPen className="text-primary-clr cursor-pointer mt-1"  /></Link>
                      <MdDelete size={20} className="text-primary-clr cursor-pointer" onClick={() => handleDeleteInventory(inventory?._id ? inventory._id : '')}/>
                    </div>
                  </TableCell>
                </TableRow>
              ))}
