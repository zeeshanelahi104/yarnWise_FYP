  ...
  const handleSave = async () => {
    try {
      if (roleId) {
        ...
        // const url = `http://localhost:3000/api/getAllUsersByRoleName?role=${role.role}` || `https://yarnwise209.vercel.app/api/getAllUsersByRoleName?role=${role.role}`;
        const url = `https://yarnwise209.vercel.app/api/getAllUsersByRoleName?role=${role.role}`;
        let postData = {
          role: role?.role,
        };
